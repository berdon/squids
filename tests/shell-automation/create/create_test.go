...
func TestCreateShellAutomation(t *testing.T) {
	bash := requireBash(t)
	...
	scriptPath := filepath.Join(repoRoot, "tests", "shell-automation", "create", "test.sh")
	...
	out, err := cmd.CombinedOutput()
	if err != nil {
		t.Fatalf("create shell automation failed: %v\n%s", err, out)
	}
}
