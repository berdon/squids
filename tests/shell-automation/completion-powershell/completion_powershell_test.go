package completionpowershell_test
...
func TestCompletionPowershellShellAutomation(t *testing.T) {
	bash := requireBash(t)
	...
	scriptPath := filepath.Join(repoRoot, "tests", "shell-automation", "completion-powershell", "test.sh")
	...
	out, err := cmd.CombinedOutput()
	if err != nil {
		t.Fatalf("completion powershell shell automation failed: %v\n%s", err, out)
	}
}
