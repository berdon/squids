...
func TestCompletionShellAutomation(t *testing.T) {
	bash := requireBash(t)
	...
	scriptPath := filepath.Join(repoRoot, "tests", "shell-automation", "completion", "test.sh")
	...
	out, err := cmd.CombinedOutput()
	if err != nil {
		t.Fatalf("completion shell automation failed: %v\n%s", err, out)
	}
}
