...
func TestCountShellAutomation(t *testing.T) {
	bash := requireBash(t)
	...
	scriptPath := filepath.Join(repoRoot, "tests", "shell-automation", "count", "test.sh")

	cmd := exec.Command(bash, scriptPath)
	cmd.Dir = repoRoot
	cmd.Env = append(os.Environ(), "SQ_BIN="+sqBin, "REPO_ROOT="+repoRoot)
	out, err := cmd.CombinedOutput()
	if err != nil {
		t.Fatalf("count shell automation failed: %v\n%s", err, out)
	}
}
