...
func TestDepAddShellAutomation(t *testing.T) {
	bash := requireBash(t)
	...
	scriptPath := filepath.Join(repoRoot, "tests", "shell-automation", "dep-add", "test.sh")
	...
	out, err := cmd.CombinedOutput()
	if err != nil {
		t.Fatalf("dep add shell automation failed: %v\n%s", err, out)
	}
}
