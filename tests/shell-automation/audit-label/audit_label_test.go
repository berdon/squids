...
func TestAuditLabelShellAutomation(t *testing.T) {
	bash := requireBash(t)
	...
	scriptPath := filepath.Join(repoRoot, "tests", "shell-automation", "audit-label", "test.sh")
	...
	out, err := cmd.CombinedOutput()
	if err != nil {
		t.Fatalf("audit-label shell automation failed: %v\n%s", err, out)
	}
}
