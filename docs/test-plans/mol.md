...
## Scope
Validate the `sq mol` command/subcommand tuple end to end, including setup requirements, success cases, error handling, output format checks, and cleanup. This plan is intended to support repeatable manual or automated verification without relying on project-global state.
...
1. Run `sq help mol` if applicable.
2. Run `sq mol --help`.
...
### 6. Concrete command checks for `sq mol`
Use the following command-focused checks as a starting point:
1. Run `sq mol --help` and record the observed output/exit code.
2. Run `sq mol` and record the observed output/exit code.
3. Run `sq mol --json` and record the observed output/exit code.


### 7. Cross-command follow-up checks
1. If `sq mol` creates or mutates sq data, validate the result with a secondary command such as `sq show`, `sq list`, `sq count --json`, or `sq status --json`.
2. If `sq mol` is documentation- or reporting-oriented, compare default output and help output for consistency.
...
## Notes
- File name for this plan follows the required convention: `docs/test-plans/mol.md`.
- Target tuple: `sq mol`.
