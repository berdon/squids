...
## Scope
Validate the `sq types` command/subcommand tuple end to end, including setup requirements, success cases, error handling, output format checks, and cleanup. This plan is intended to support repeatable manual or automated verification without relying on project-global state.
...
1. Run `sq help types` if applicable.
2. Run `sq types --help`.
...
### 6. Concrete command checks for `sq types`
Use the following command-focused checks as a starting point:
1. Run `sq types --help` and record the observed output/exit code.
2. Run `sq types` and record the observed output/exit code.
3. Run `sq types --json` and record the observed output/exit code.


### 7. Cross-command follow-up checks
1. If `sq types` creates or mutates sq data, validate the result with a secondary command such as `sq show`, `sq list`, `sq count --json`, or `sq status --json`.
2. If `sq types` is documentation- or reporting-oriented, compare default output and help output for consistency.
...
## Notes
- File name for this plan follows the required convention: `docs/test-plans/types.md`.
- Target tuple: `sq types`.
