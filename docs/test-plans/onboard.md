# Test Plan: `sq onboard`

## Scope
Validate the `sq onboard` command/subcommand tuple end to end, including setup requirements, success cases, error handling, output format checks, and cleanup. This plan is intended to support repeatable manual or automated verification without relying on project-global state.
...
1. Run `sq help onboard` if applicable.
2. Run `sq onboard --help`.
...
### 6. Concrete command checks for `sq onboard`
Use the following command-focused checks as a starting point:
1. Run `sq onboard --help` and record the observed output/exit code.
2. Run `sq onboard` and record the observed output/exit code.
3. Run `sq onboard --json` and record the observed output/exit code.


### 7. Cross-command follow-up checks
1. If `sq onboard` creates or mutates sq data, validate the result with a secondary command such as `sq show`, `sq list`, `sq count --json`, or `sq status --json`.
2. If `sq onboard` is documentation- or reporting-oriented, compare default output and help output for consistency.
...
## Notes
- File name for this plan follows the required convention: `docs/test-plans/onboard.md`.
- Target tuple: `sq onboard`.
