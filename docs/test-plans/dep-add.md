# Test Plan: `sq dep add`

## Scope
Validate the `sq dep add` command/subcommand tuple end to end, including setup requirements, success cases, error handling, output format checks, and cleanup. This plan is intended to support repeatable manual or automated verification without relying on project-global state.
...
1. Run `sq help dep add` if applicable.
2. Run `sq dep add --help`.
...
### 6. Concrete command checks for `sq dep add`
Use the following command-focused checks as a starting point:
1. Run `sq dep add --help` and record the observed output/exit code.
2. Run `sq dep add` and record the observed output/exit code.
3. Run `sq dep add --json` and record the observed output/exit code.


### 7. Cross-command follow-up checks
1. If `sq dep add` creates or mutates sq data, validate the result with a secondary command such as `sq show`, `sq list`, `sq count --json`, or `sq status --json`.
2. If `sq dep add` is documentation- or reporting-oriented, compare default output and help output for consistency.
...
## Notes
- File name for this plan follows the required convention: `docs/test-plans/dep-add.md`.
- Target tuple: `sq dep add`.
