# Test Plan: `sq gate show`

## Scope
Validate the `sq gate show` command/subcommand tuple end to end, including setup requirements, success cases, error handling, output format checks, and cleanup. This plan is intended to support repeatable manual or automated verification without relying on project-global state.
...
1. Run `sq help gate show` if applicable.
2. Run `sq gate show --help`.
...
### 6. Concrete command checks for `sq gate show`
Use the following command-focused checks as a starting point:
1. Run `sq gate show --help` and record the observed output/exit code.
2. Run `sq gate show` and record the observed output/exit code.
3. Run `sq gate show --json` and record the observed output/exit code.


### 7. Cross-command follow-up checks
1. If `sq gate show` creates or mutates sq data, validate the result with a secondary command such as `sq show`, `sq list`, `sq count --json`, or `sq status --json`.
2. If `sq gate show` is documentation- or reporting-oriented, compare default output and help output for consistency.
...
## Notes
- File name for this plan follows the required convention: `docs/test-plans/gate-show.md`.
- Target tuple: `sq gate show`.
